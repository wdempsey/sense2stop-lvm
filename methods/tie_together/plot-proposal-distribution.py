...
# Choose combo with uniform probability across all possible types of combo
which_combo = np.random.choice(['smart_birth','smart_death'], 1, p=[1,0])

if which_combo == 'smart_birth':
    ...
    which_move_type = np.random.choice(['add_point','delete_point'], 1, p=[0,1])
    ...
    if which_move_type == 'add_point':
        ...
        #######################################################################
        # Plot CDF of smart birth proposal distribution
        #######################################################################
        ...
        #######################################################################
        # Plot PDF of smart birth proposal distribution
        #######################################################################
        plt.figure(clear=True)
        plt.title('Smart Birth')
        plt.xlim(left=-0.5, right= latent_obj.latent_data['day_length'] + 0.75)
    ...
    # forward move proposal distribution is smart death, 
    # reverse move proposal distribution is dumb birth
    pass
